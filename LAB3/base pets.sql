...
---- TABLAS CREADAS POR JAVIER ANDR…S TORALES ----
...
---- TABLAS CREADAS POR EZEQUIEL ALEJANDRO MARTÌNEZ ----
...
---- TABLAS CREADAS POR MARÕA OLIVIA HANCZYC  ----
...
----------------------------------------------------------------------------------------------------------------------------------------------
/*-- PROCEDIMIENTOS CREADOS POR M¡XIMO CANEDO ---*/
--- PROCEDIMIENTO ALMACENADO
-- DescripciÛn: AÒade un registro a la tabla Empleados.
...
--- PROCEDIMIENTO ALMACENADO
-- DescripciÛn: Cambia el hash y el salt de un empleado en particular.
-- Uso: P·gina /Empleados/CambiarClave.aspx.
...
--- PROCEDIMIENTO ALMACENADO
-- DescripciÛn: Cambia el sueldo de un registro Empleado. Devuelve 1 si se pudo, o 0 si no.
...
--- PROCEDIMIENTO ALMACENADO
-- DescripciÛn: Deshabilita un registro Empleado. Devuelve 1 si se pudo, o 0 si no.
...
--- PROCEDIMIENTO ALMACENADO
-- DescripciÛn: Devuelve la cantidad y los detalles del producto m·s vendido en los ˙ltimos siete dÌas.
-- Observaciones: Dado que el servidor usa la hora de Greenwich y no es posible cambiarlo, les restÈ 3 horas a los c·lculos.
-- Uso: Widget de Productos en p·gina de Inicio (/Inicio.aspx).
...
--- PROCEDIMIENTO ALMACENADO
-- DescripciÛn: Devuelve el total de las ventas de las ˙ltimas 24 horas.
-- Observaciones: Dado que el servidor usa la hora de Greenwich y no es posible cambiarlo, les restÈ 3 horas a los c·lculos.
-- Uso: Widget de Ventas en p·gina de Inicio (/Inicio.aspx).
...
--- PROCEDIMIENTO ALMACENADO
-- DescripciÛn: Devuelve el total de las ventas de los ˙ltimos siete dÌas.
-- Observaciones: Dado que el servidor usa la hora de Greenwich y no es posible cambiarlo, les restÈ 3 horas a los c·lculos.
-- Uso: Widget de Ventas en p·gina de Inicio (/Inicio.aspx).
...
--- PROCEDIMIENTO ALMACENADO
-- DescripciÛn: Devuelve la cantidad de productos que cuentan con stock menor a cinco.
-- Uso: Widget de Productos en p·gina de Inicio (/Inicio.aspx).
...
--- PROCEDIMIENTO ALMACENADO
-- DescripciÛn: Devuelve la cantidad de productos que cuentan con stock cero.
-- Uso: Widget de Productos en p·gina de Inicio (/Inicio.aspx).
...
--- PROCEDIMIENTO ALMACENADO
-- DescripciÛn: Crea un registro en la tabla Ventas, y devuelve una tabla con el ID del nuevo registro y la cantidad de filas modificadas. Esta ˙ltima se usa para verificar en el cÛdigo que sÌ se haya podido ejecutar el INSERT.
...
CREATE PROCEDURE IniciarVenta
    ...
    BEGIN
        ...
        -- Obtenemos el n˙mero de filas modificadas
...
----------------------------------------------------------------------------------------------------------------------------------------------
/*-- PROCEDIMIENTOS CREADOS POR JAVIER ANDR…S TORALES ---*/
...
----------------------------------------------------------------------------------------------------------------------------------------------
/*-- PROCEDIMIENTOS CREADOS POR MARÕA OLIVIA HANCZYC  ---*/
...
--NOTA: este procedimiento (SP_DetalleDeVentas_DarDeBaja) fue realizado tambiÈn por Ezequiel Alejandro MartÌnez:
...
IF(@Stock > 0) --Si el stock es mayor a cero, entonces:
	BEGIN
		...
		UPDATE Productos
		SET Stock_Prod = Stock_Prod + 1 --Se actualiza el stock en la tabla Productos, sum·ndole una unidad (ya que se vende menos cantidad).
...
IF(@Stock > 0) --Si el stock es mayor a cero, entonces:
	BEGIN
		...
		UPDATE Productos
		SET Stock_Prod = Stock_Prod - 1 --Se actualiza el stock en la tabla Productos, rest·ndole una unidad (ya que se vende m·s cantidad).
		WHERE CodProducto_Prod = @CodigoProducto AND CUITProveedor_Prod = @CUITProveedor
...
----------------------------------------------------------------------------------------------------------------------------------------------
/*-- PROCEDIMIENTOS CREADOS POR EZEQUIEL ALEJANDRO MARTÌNEZ  ---*/
...
/**** Triggers ****/
/*--- TRIGGER CREADO POR MARÕA OLIVIA HANCZYC  ---*/
...
/*-- TRIGGER CREADO POR EZEQUIEL ALEJANDRO MARTÌNEZ  ---*/
...
/*--- TRIGGER CREADO POR JAVIER ANDR…S TORALES ---*/
CREATE TRIGGER TR_Proveedores_PrevenirEliminar
ON Proveedores
INSTEAD OF DELETE
AS
BEGIN
    PRINT('No est· permitido eliminar registros de la tabla "Proveedores" mediante el uso de "Delete".');
    ROLLBACK;
END;
go

---- Integrantes ----
select  upper('Javier Andres Torales'), upper('MarÌa Olivia Hanczyc '),upper('Ezequiel Alejandro Martinez'),upper('M·ximo Canedo'),upper('jean esquen')