...
namespace Datos {
    public class DaoTiposDeProductos {
        ...
        /// </summary>
        /// <returns>Objeto Response con el resultado de la operación. </returns>
        public static Response ObtenerListaDeTipoProductoActivos()
        {
            Connection connection = new Connection(Connection.Database.Pets);
            return connection.FetchData(
                        query: $"SELECT {ALL_COLUMNS}, [{Animal.Columns.Nombre}] FROM [{TipoProducto.Table}] INNER JOIN [{Animal.Table}] ON [{TipoProducto.Columns.CodAnimal}] = [{Animal.Columns.Codigo}] where [{TipoProducto.Columns.Estado}] = 1 "
                    );
        }
